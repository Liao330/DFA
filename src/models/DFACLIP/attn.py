...
class RecAttnClip(nn.Module):
    ...
    def forward(self, data_dict, clip_features, attn_bias,inference=False, normalize=False):
        ...
        # print(f"clip_features[self.first_layer]:{clip_features[self.first_layer].shape}") # [16, 256, 1024]
        vision_tokens = clip_features[self.first_layer].permute(1, 0, 2).clone()  # NLD->LND
        # clss_nums = self.clss_nums + 68 # 128 -》 196
        ...
        # print(f"attn_bias:{attn_bias.shape}") # [64, 16, 128, 16, 16]
        attn_biases = self.build_attn_mask(attn_bias)
        # print(f"attn_biases:{attn_biases[0].shape}") # [1024, 385, 385]
        ...
        # print(f"111:{clss_token.shape}") # [128, 16, 1024]
        clss_token = x[:, :self.clss_nums, :]
        # print(f"222:{clss_token.shape}") # [16, 196-68, 1024]
