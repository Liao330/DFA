...
class Xception(nn.Module):
    ...
    def forward(self, x, inference=False):
        ...
        # 提取全局平均池化后的特征
        features = F.adaptive_avg_pool2d(x, (1, 1))  # [B, 2048, 1, 1]
        ...
        if inference:
            return {
                'logits': logits,
                'features': features  # 返回中间特征用于 T-SNE
            }
        return logits
...
if __name__ == "__main__":
    ...
    output = model(input_tensor, inference=True)
    print("Logits shape:", output['logits'].shape)  # 应输出 torch.Size([8, 2])
    print("Features shape:", output['features'].shape)  # 应输出 torch.Size([8, 2048])