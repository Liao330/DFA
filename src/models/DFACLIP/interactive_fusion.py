...
class Interactive_fusion_classifier(nn.Module):
    ...
    def forward(self, clip_fmp, guide_fmp, return_features=False):
        if guide_fmp is None:
            fused_seq_pooled = clip_fmp.mean(dim=1)  # [B, 1024]
            logits = self.classifier(fused_seq_pooled)
            if return_features:
                return logits, fused_seq_pooled
        else:
            B = guide_fmp.shape[0]
