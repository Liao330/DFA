...
from src.config import BATCH_SIZE, IMAGE_SIZE, CRITERION, NUM_CLASS
...
class EfficientViT(nn.Module):
    ...
    def forward(self, img, mask=None):
        ...
        # x = self.features(img)
        # print(f"111:{x.shape}") # [64, 1280, 7, 7]
        # x2 = self.features(img)
        y = rearrange(x, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=p, p2=p)
        # y2 = rearrange(x2, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = p, p2 = p)
        y = self.patch_to_embedding(y)
        # print(f"yyy:{y.shape}") # [64, 1, 1024]
        cls_tokens = self.cls_token.expand(x.shape[0], -1, -1)
        # print(f"cls:{cls_tokens.shape}") # [64, 1, 1024]
        x = torch.cat((cls_tokens, y), 1)
        # print(f"222:{x.shape}") # [64, 2, 1024]
        shape = x.shape[0]
        x += self.pos_embedding[0:shape]
        # print(f"333:{x.shape}")
        x = self.dropout(x)
        # print(f"444:{x.shape}") # [64, 2, 1024]
        x = self.transformer(x)
        # print(f"555:{x.shape}") # [64, 2, 1024]
        x = self.to_cls_token(x[:, 0])
        # print(f"666:{x.shape}") # [64, 1024]
        return self.mlp_head(x)

# test
# img = torch.randn(64,3,224,224).cuda()
# model = EfficientViT().cuda()
# out = model(img)
# print(out)